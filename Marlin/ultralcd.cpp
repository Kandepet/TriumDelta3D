...
millis_t next_lcd_fullupdate_ms;
...
void kill_screen(const char* lcd_msg) {
  ...
  static void lcd_prepare_menu() {
    ...
    static void lcd_delta_calibrate_menu() {
      ...
      MENU_ITEM(back, MSG_MAIN);
      MENU_ITEM(gcode, MSG_AUTO_HOME, PSTR("G28"));
      MENU_ITEM(gcode, MSG_DELTA_CALIBRATE_X, PSTR("G0 F5000 X-38.97 Y-22.5 Z10"));
      MENU_ITEM(gcode, MSG_DELTA_CALIBRATE_Y, PSTR("G0 F5000 X38.97 Y-22.5 Z10"));
      MENU_ITEM(gcode, MSG_DELTA_CALIBRATE_Z, PSTR("G0 F5000 X0 Y45 Z10"));
      MENU_ITEM(gcode, MSG_DELTA_CALIBRATE_CENTER, PSTR("G0 F5000 X0 Y0 Z0"));
      END_MENU();
    }
...
void lcd_update() {
  ...
  if (ELAPSED(ms, next_lcd_update_ms)) {
    ...
    static int8_t lcd_status_update_delay = 0; // first update one loop delayed
    if (
      #if ENABLED(ULTIPANEL)
        currentScreen == lcd_status_screen &&
      #endif
  ...
  if (ELAPSED(ms, next_lcd_fullupdate_ms)) {
  
  next_lcd_fullupdate_ms = ms + LCD_FULLREDRAW_INTERVAL;
